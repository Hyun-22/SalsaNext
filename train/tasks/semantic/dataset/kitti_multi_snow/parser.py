  ...
  def __init__(self, root,    # directory where data is
    ...
    if db_type == "train":
        for idx, data_path in enumerate(self.root):
            ...
            crop_len = len(tmp_pcd_files) // len(self.root)
            self.scan_files += tmp_pcd_files[crop_len * idx:crop_len * (idx + 1)]
        ...
        for idx, data_path in enumerate(self.root):
            ...
            tmp_label_files.clear()
            print(len(self.scan_files))
        print("len in snow 0 : {}".format(len(self.scan_files)))
        # print("validation sample")
        # print(self.scan_files[:10])
